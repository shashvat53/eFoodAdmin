...
const SideBar = ({ product, openNav, makeOpen, setMakeOpen }) => {
  ...
  const sideBarFunction = () => {
   return <motion.div
      className={`sidebar-scrollbar absolute bg-white top-full h-[90vh] gap-6 border-r-2 border-gray-100 z-[-1] pt-4 overflow-y-scroll w-[250px] ${!makeOpen && openNav !== 'sm' && 'w-[80px] overflow-y-hidden' }`}
      initial={{ width: 80 }}
        ...
        ? product.map((elem, index) => {
            return (
              <ul className={`p-3 w-full`} key={index}>
                <p className={`text-gray-500 text-xs pl-3  border-b border-gray-50 my-2 uppercase ${!makeOpen && openNav !== 'sm' && 'hidden' }`}>
                  {elem.cate}
                </p>
                {elem.item?.map((ite, num) => {
                  return (
                    ...
                    >
                      {ite.href ? (
                        <Link to={ite.href} className="flex gap-2 items-center">
                          {ite.icon}
                          <span className={`${!makeOpen && openNav !== 'sm' && 'hidden' }`}>{ite.name}</span>
                        </Link>
                      ) : (
                        <ul className="flex flex-col gap-2 items-start relative">
                          ...
                          >
                            ...
                            <span
                              id="openSmallMenu"
                              className={` cursor-pointer ${!makeOpen && openNav !== 'sm' && 'hidden' }`}
                            >
                              {ite.name}
                            </span>
                            <svg
                              ...
                              {ite.moreOption !== false &&
                                ite.moreOption.map((lab, num2) => {
                                  return (
                                    <Link
                                      key={num2}
                                      to={`${ite.name.replaceAll(' ', '')}/${lab.label.replaceAll(' ', '')}`}
                                      className="flex gap-2 items-center"
                                    >
                                      <span
                                        className={` mb-3  text-sm ${
                                          isActive === "/" + (ite.name.replaceAll(' ', '')+'/'+lab.label.replaceAll(' ', ''))
                                            ? "text-red-400"
                                            : "text-gray-800"
                                        }`}
                                      >
