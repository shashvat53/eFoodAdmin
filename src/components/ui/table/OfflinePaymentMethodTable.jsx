...
const OfflinePaymentMethodTable = ({ data }) => {
  const listArr = [
    "SL",
    "Payment Method Name",
    "Payment Info",
  ...
  const Item = (elem, ind) => {
    return (
      <>
        <span >{ind + 1}</span>
        <span>{elem.PaymentMethodName}</span>
        <span>{elem.PaymentInfo}</span>
        <span>{elem.RequiredInfoFromCustomer}</span>
...
export default OfflinePaymentMethodTable;
