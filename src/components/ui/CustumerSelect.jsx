...
const CustomDropdown = ({ options, onSelect }) => {
  ...
  return (
    <div className="relative inline-block text-left w-full min-w-[250px] px-5 py-2 border-2 rounded-md h-12 " ref={dropdownRef}>
      <div>
        <div className="rounded-md shadow-sm w-full" onClick={() => setIsOpen(!isOpen)}>
         Select 
        </div>
      </div>
      {isOpen && (
      ...
      {selectedOption && <p className="mt-2 hidden">Selected: {selectedOption}</p>}
    </div>
  );
};
